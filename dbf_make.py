...
DICT_FIELDS = {
    ...
    'N': 'ID C(8);IDN C(8);DAT C(8);NN C(13);NAIM C(255);KOL C(6);NCEN C(7);SUM C(9);PROD C(35);OTDEL C(30);KOM C(50);'
         'CENA2 C(20);CENA3 C(20);ARTNAME C(100);ARTNOM C(30);KATNAME C(50);KATNOM C(30);NDS N(2, 0);GTD C(255);COUNTRY C(50);'
...
def main(my_rec):
    try:
        ...
        logger_make_dbf.info(f'создаем z файл')
        dbf_z(i_path, name_export, my_rec, id_number)
        logger_make_dbf.info(f'создаем n файл')
        dbf_n(i_path, name_export, my_rec, id_number)
        end: float = time.time()
        logger_make_dbf.info('Done in {:.4}'.format(end - start))
